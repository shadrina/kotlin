...
import org.jetbrains.kotlin.types.typeUtil.upperBound
...
class AdditionalReceiverChecker : DeclarationChecker {
    override fun check(declaration: KtDeclaration, descriptor: DeclarationDescriptor, context: DeclarationCheckerContext) {
        ...
        for ((i, receiver) in additionalReceivers.withIndex()) {
            val receiverType = receiver.type.upperBound().withTypeParametersErased()

            for (another in additionalReceivers.subList(i + 1, additionalReceivers.size)) {
                val anotherReceiverType = another.type.upperBound().withTypeParametersErased()
